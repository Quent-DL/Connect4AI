...
typedef struct game {
    uint64_t gridA;
    uint64_t gridB;
    char cols_occupation[ROW_LENGTH];
} game_t;
 ...
 * 
 * @param game the game to which apply the move and (if successful) the results
 ...
 * 
 * @returns The new (updated) game if the move is valid, and 'game' is left untouched
